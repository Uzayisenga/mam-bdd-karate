...
public class Endpoints {
    ...
    @LazyAutowired
    private Member member; // This member is directly autowired
    ...
    public Response updateUser_2FA(){
        ...
        response = given()
                ...
                .when()
                .put(dataProvider.getBaseHost() + "/v1/user/"+member.getCustNo()); // Using autowired 'member' directly
        // REPLACED: JUnit assertion with a runtime check.
    ...
    public Response getMobileContactPoints(){
        System.out.println(member.getCustNo()); // Using autowired 'member' directly
        response = given()
                .header("Origin", dataProvider.getBaseOrigin())
                .cookies(dataProvider.getCookies())
                .when()
                .get(dataProvider.getBaseHost() + "/v1/user/"+member.getCustNo()+"/contactpoint"); // Using autowired 'member' directly
    ...
    public Response requestTAN_usingMobileNumber(){
        ...
        response = given()
                ...
                .contentType("application/json")
                .cookies(dataProvider.getCookies())
                .queryParam("intPrefix", member.getInternationalDialingPrefix()) // Using autowired 'member' directly
                .queryParam("localNr", member.getLocalNumber()) // Using autowired 'member' directly
                .queryParam("userId", member.getCustNo()) // Using autowired 'member' directly
                .post(dataProvider.getBaseHost() +"/validation/v1/testing/me/sms");
        // REPLACED: JUnit assertion with a runtime check.
        if (response.statusCode() != 201) {
    ...
    public Response getCustNo(String email) {
        response = given()
                ...
                .get(dataProvider.getBaseHost() +"/v1/user/custno");
...
//              .queryParam("userId", member.getCustNo()) // Commented out
    ...
    public Response GetDetails(){
        ...
        //      .get(dataProvider.getBaseHost() +"/v3/member/me/pin");
        //        .get(dataProvider.getBaseHost() +"/v1/member/me");
    ...
    public Response GetDetails_UAT2(){
        System.out.println(dataProvider.getUAT2_BaseOrigin());
        ...
        response = given()
                ...
                .cookies(dataProvider.getCookies())
                .when()
                .get(dataProvider.getUAT2_BaseHost() +"/member/v3/me");
        System.out.println(dataProvider);
        // REPLACED: JUnit assertion with a runtime check.
        if (response.statusCode() != 200) {
            throw new RuntimeException("GetDetails_UAT2 failed: Expected status code 200 but got " + response.statusCode() + ". Response body: " + response.getBody().asString());
        }
        return response;
    }
    ...
    public Response userEnebledAllInOneUser() {
        ...
        response = given()
                ...
                .when()
                .put(dataProvider.getBaseHost() + "/v1/user/"+member.getCustNo()+"/permission/all-in-one"); // Using autowired 'member' directly
//                .put(dataProvider.getBaseHost() + "/v1/user/me/permission/all-in-one");
        System.out.println("Response Code "+ response.statusCode());
        System.out.println("Body "+ response.getBody().asString());
        return response;
    }
...
//                .body( jsonObjectGenerator.getJsonBody(jsonObjectGenerator.createAllInOneBody()) )
                .when()
                .get(dataProvider.getBaseHost() + "/v1/user/"+member.getCustNo()+"/permission"); // Using autowired 'member' directly
//                .put(dataProvider.getBaseHost() + "/v1/user/me/permission/all-in-one");
        System.out.println("Response Code "+ response.statusCode());
        System.out.println("Body "+ response.getBody().asString());
        System.out.println("----------------");
...
//                .body( jsonObjectGenerator.getJsonBody(jsonObjectGenerator.createAllInOneBody()) )
                .when()
                .get(dataProvider.getBaseHost() + "/v1/user/"+member.getCustNo()+"/subscription"); // Using autowired 'member' directly
//                .put(dataProvider.getBaseHost() + "/v1/user/me/permission/all-in-one");
        System.out.println("Response Code "+ response.statusCode());
        System.out.println("Body "+ response.getBody().asString());
        return response;
